...
export class E621 {
  ...
  //#region private
  private takeWhen(post: Post, when: { included?: string[]; excluded?: string[]; }): boolean {
    const whenIncluded = when.included ?? [];
    const whenExcluded = when.excluded ?? [];

    return !(whenIncluded.every(tag => post.tags.all.includes(tag)) && whenExcluded.every(tag => !post.tags.all.includes(tag)));
  }
  //#endregion
}
