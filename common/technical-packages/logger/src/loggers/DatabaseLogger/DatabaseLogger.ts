import * as logsService from '@zougui/log-service';
...
export class DatabaseLogger extends BatchLogger<LoggerDatabaseConfig> {
  ...
  private _logsService: typeof logsService = require('@zougui/log-service');
  ...
  protected async sendLogs(logs: BatchedLog[]): Promise<void> {
    const results = await this._logsService.log.createMany(logs.map(log => log.log));
